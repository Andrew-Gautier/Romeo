...
Launch with: torchrun --nproc_per_node=8 train_lstm_ddp2.py [args]

EXPERIMENT 2: Uses HuggingFace model weights (DeepSeek Coder / CodeLlama)
"""
...
# Default config - vocab_size and embedding_size will be overridden by HuggingFace model config
DEFAULT_CONFIG = {
    'batch_size': 16,  # Per-GPU batch size (effective = batch_size * num_gpus)
    'learning_rate': 0.001,
    'epochs': 50,
    'lstm_nodes': 256,
    'vocab_size': 32256,      # Will be updated from HF model config
    'embedding_size': 4096,   # Will be updated from HF model config
    'output_dim': 1,
    'patience': 5,
    'gradient_clip': 1.0,
    'n_layers': 2,
...
# Supported HuggingFace models and their embedding key names
HF_MODEL_CONFIGS = {
    'deepseek-coder': {
...
# ============================================================================
# HuggingFace Weight Loading Utilities
# ============================================================================

def find_hf_model_path(cache_dir, model_name):
    """
    Find the actual model files in the HuggingFace cache directory.
    
    Args:
        cache_dir: Path to HuggingFace cache (e.g., /scratch/aeg00011/huggingface)
        model_name: Model identifier (e.g., 'deepseek-coder' or 'codellama')
    
    Returns:
        Path to the model snapshot directory containing the actual files
    """
...
def load_hf_embeddings(cache_dir, model_name):
    ...
    print(f"Model config: vocab_size={vocab_size}, hidden_size={embedding_dim}")
    
    # Find and load the safetensors file(s)
    safetensor_files = glob.glob(os.path.join(model_path, '*.safetensors'))
    if not safetensor_files:
        # Try loading from pytorch .bin files as fallback
        bin_files = glob.glob(os.path.join(model_path, 'pytorch_model*.bin'))
        if bin_files:
            return load_hf_embeddings_from_bin(bin_files, model_name, vocab_size, embedding_dim)
    ...
    for sf_file in safetensor_files:
        print(f"  Checking {os.path.basename(sf_file)}...")
        try:
            # Load only the embedding weights to save memory
    ...
    if embeddings is None:
        # If not found with exact key, try to find any embedding-related key
        for sf_file in safetensor_files:
            state_dict = load_safetensors(sf_file)
            for key in state_dict.keys():
                if 'embed' in key.lower() and 'token' in key.lower():
                    print(f"  Found alternative embedding key: {key}")
    ...
    if embeddings is None:
        raise KeyError(f"Could not find embedding weights. Tried key: {embedding_key}")
    
    return embeddings, vocab_size, embedding_dim


def load_hf_embeddings_from_bin(bin_files, model_name, vocab_size, embedding_dim):
    """Fallback: Load embeddings from pytorch .bin files."""
    embedding_key = HF_MODEL_CONFIGS[model_name]['embedding_key']
    
    for bin_file in bin_files:
        print(f"  Checking {os.path.basename(bin_file)}...")
    ...
    raise KeyError(f"Could not find embedding weights in .bin files")
...
def run_experiment(
    ...
    Args:
        dataset_name: Name of the dataset (e.g., 'juliet_c_simhash_k=1')
        dataset_dir: Path to training dataset
        ood_dir: Path to OOD evaluation dataset
        output_dir: Output directory for results
        hf_cache_dir: Path to HuggingFace cache directory
        model_name: Name of the model ('deepseek-coder' or 'codellama')
    ...
    print_rank0(f"Loading pretrained embeddings from HuggingFace model: {model_name}")
    print_rank0(f"Cache directory: {hf_cache_dir}")
    pretrained_weights, vocab_size, embedding_dim = load_hf_embeddings(hf_cache_dir, model_name)
    print_rank0(f"Loaded embeddings: shape={pretrained_weights.shape}, vocab={vocab_size}, dim={embedding_dim}")
    
    # Update config with actual model dimensions
    config['vocab_size'] = vocab_size
    config['embedding_size'] = embedding_dim
    print_rank0(f"Updated config: vocab_size={config['vocab_size']}, embedding_size={config['embedding_size']}")
...
def main():
    parser = argparse.ArgumentParser(description='Train LSTM with DDP for vulnerability detection (HuggingFace embeddings)')
    ...
    parser.add_argument('--hf-cache-dir', type=str, required=True,
                        help='Path to HuggingFace cache directory containing model weights')
    parser.add_argument('--model-name', type=str, required=True, choices=list(HF_MODEL_CONFIGS.keys()),
                        help=f'HuggingFace model to use: {list(HF_MODEL_CONFIGS.keys())}')
